(() => {
    ...
    window.onload = function() {
        ...
        function initBlog(data, startItem, endItem) {
            let dataPart = data.items.slice(startItem, endItem);
            dataPart.forEach((element => {
                buildBlogItem(element);
            }));
        }
        function buildBlogItem(item) {
            let blockItemTemplete = ``;
            blockItemTemplete += ` <article data-id='${item.id}' class="blog__item item-blog">`;
            item.image ? blockItemTemplete += `\n         <a href="${item.url}" class="item-blog__imamge-ibg"><img src="${item.image}" alt="" /></a>\n      ` : null;
            item.data ? blockItemTemplete += `\n      <div class="item-blog__date">${item.data}</div>\n      ` : null;
            item.title ? blockItemTemplete += `\n      <h4 class="item-blog__title">\n         <a href="${item.url}" class="item-blog__link-title">${item.title}</a>\n      </h4>\n            ` : null;
            item.text ? blockItemTemplete += `\n      <div class="item-blog__text text">\n         ${item.text}\n      </div>\n      ` : null;
            if (item.tags) {
                blockItemTemplete += `<div class="item-blog__tags">`;
                for (const tag in item.tags) blockItemTemplete += `\n            <a href="${item.tags[tag]}" class="item-blog__tag">${tag}</a>\n            `;
                blockItemTemplete += `</div>`;
            } else null;
            blockItemTemplete += `</article>`;
            blogItems.insertAdjacentHTML("beforeend", blockItemTemplete);
        }
        document.addEventListener("click", documentActions);
        function documentActions(el) {
            const targetEl = el.target;
            if (targetEl.closest(".blog__view-more")) {
                startItem = endItem;
                endItem = startItem + 3;
                initBlog(data, startItem, endItem);
                el.preventDefault();
